...
function getSelectedWeekdays() {
  const checkboxes = document.querySelectorAll(
    'input[name="repeat-weekday"]:checked'
  );
...
async function saveRepeatGroupAndReservations({
  ...
  alert(
    `定例予約「${title}」を登録しました。${reservations.length}件の予約が作成されました。`
  );
}

// Googleカレンダーに登録する関数
async function registerGoogleCalendarEvent({
  room,
  username,
  type,
  start,
...
async function renderRepeatGroups() {
  ...
  snapshot.forEach((doc) => {
    ...
    // 表示用の曜日
    const weekdaysStr = data.weekdays
      .map((n) => ["日", "月", "火", "水", "木", "金", "土"][n])
      .join("・");

    const div = document.createElement("div");
    div.className =
      "border p-3 rounded shadow-sm flex justify-between items-center";
    div.innerHTML = `
      <div>
        <div class="font-bold">${data.title}</div>
        <div class="text-sm text-gray-600">毎週 ${weekdaysStr}曜日 ${
      data.timeFrom
    }〜${data.timeTo}</div>
        <div class="text-xs text-gray-400">月数: ${
          data.months || "?"
        }ヶ月 / 登録数: ${data.count || 0}件</div>
      </div>
      <button onclick="deleteRepeatGroup('${
        doc.id
      }')" class="text-red-600 hover:underline text-sm">🗑️ 削除</button>
    `;
  ...
  if (!hasValidGroup) {
    list.innerHTML =
      '<p class="text-gray-500">登録された定例予約はありません。</p>';
  }
}
